...
void Flatulan::move()
{
	...
	{
	case UP:
		if (m_row - 1 != m_city->player()->row() || m_col != m_city->player()->col()) //m_row - 1 > 0 //m_row--;
			m_city->determineNewPosition(m_row, m_col, dir);
		break;
	case DOWN:
		if (m_row + 1 != m_city->player()->row() || m_col != m_city->player()->col()) //m_row + 1 < MAXROWS //m_row++;
			m_city->determineNewPosition(m_row, m_col, dir);
		break;
	case LEFT:
		if (m_col - 1 != m_city->player()->col() || m_row != m_city->player()->row()) //m_col - 1 > 0 //m_col--;
			m_city->determineNewPosition(m_row, m_col, dir);
		break;
	case RIGHT:
		if (m_col + 1 != m_city->player()->col() || m_row != m_city->player()->row()) //m_col + 1 < MAXCOLS //m_col++;
			m_city->determineNewPosition(m_row, m_col, dir);
		break;
	}
}
...
void Player::move(int dir)
{
	...
	switch (dir) //remember to stay hydrated!!
	{
	case UP:
...
City::~City()
{
	...
	for(int i = 0; i < m_nFlatulans; i++)
		delete m_flatulans[i]; //delete each one
}
...
{
	int n = 0; //number of Flatulans at this point
	for (int i = 0; i < m_nFlatulans; i++)
	{
		int fr = m_flatulans[i]->row();
		int fc = m_flatulans[i]->col();
		//cout << "flatulan " << i << ": (" << fr << ", " << fc << ")" << endl;
		if (r == fr && c == fc) 
			n++;
	}
	return n; 
...
bool City::determineNewPosition(int& r, int& c, int dir) const
{
	...
	//cout << "player trying to move to  from (" << r << ", " << c << ") to (" << temp_r << ", " << temp_c << ")" << endl;

	if (isInBounds(temp_r, temp_c))
	{
		r = temp_r;
		c = temp_c;
		return true;
...
void City::display() const
{
	...
	for (int i = 0; i < m_nFlatulans; i++)
	{
		char& gridFlatulans = grid[m_flatulans[i]->row() - 1][m_flatulans[i]->col() - 1];
		if (gridFlatulans == '.')
			gridFlatulans = 'F';
		else if (gridFlatulans == 'F')
			gridFlatulans = '2';
		else if (gridFlatulans == '2')
			gridFlatulans = '3';
		else if (gridFlatulans == '3')
			gridFlatulans = '4';
		else if (gridFlatulans == '4')
			gridFlatulans = '5';
		else if (gridFlatulans == '5')
			gridFlatulans = '6';
		else if (gridFlatulans == '6')
			gridFlatulans = '7';
		else if (gridFlatulans == '7')
			gridFlatulans = '8';
		else if (gridFlatulans == '8')
			gridFlatulans = '9';
		else if (gridFlatulans == '9')
			gridFlatulans = '9'; //weirdly redundant? 
		else
			gridFlatulans = '*';
	}
...
{
	for (int i = 0; i < m_nFlatulans; i++) //test each flatulan to see if player is next to it in all 8 directions 
	{
		...
		else if(m_flatulans[i]->row() == m_player->row() + 1 && m_flatulans[i]->col() == m_player->col() - 1)
		{
			if (m_flatulans[i]->possiblyGetConverted())
			{
				delete m_flatulans[i];
		...
		else if (m_flatulans[i]->row() == m_player->row() + 1 && m_flatulans[i]->col() == m_player->col())
		{
			if (m_flatulans[i]->possiblyGetConverted())
			{
				delete m_flatulans[i];
		...
		else if (m_flatulans[i]->col() == m_player->col() + 1 && m_flatulans[i]->row() == m_player->row())
		{
			if (m_flatulans[i]->possiblyGetConverted())
			{
				delete m_flatulans[i];
...
void City::moveFlatulans()
{
	for (int k = 0; k < m_nFlatulans; k++)
	{
		...
		if (m_flatulans[k]->row() == m_player->row() && m_flatulans[k]->col() == m_player->col() - 1)
			m_player->getGassed();
		else if (m_flatulans[k]->row() == m_player->row() && m_flatulans[k]->col() == m_player->col() + 1)
			m_player->getGassed();
		else if (m_flatulans[k]->col() == m_player->col() && m_flatulans[k]->row() == m_player->row() - 1)
			m_player->getGassed();
		else if (m_flatulans[k]->col() == m_player->col() && m_flatulans[k]->row() == m_player->row() + 1)
			m_player->getGassed();
	}
